...
window.onload=()=>{
    ...
    i.addEventListener("click",()=>{
        ...
        //ex4
        let result= ""
        let oVert= topVert.every((x)=> x === "O") || midVert.every((x)=> x === "O") || lowVert.every((x)=>x === "O")
        let xVert= topVert.every((x)=> x === "X") || midVert.every((x)=> x === "X") || lowVert.every((x)=>x === "X")

        let xHor= top.every((x)=> x === "X") || mid.every((x)=> x === "X") || low.every((x)=>x === "X") 
        let oHor= top.every((x)=> x === "O") || mid.every((x)=> x === "O") || low.every((x)=>x === "O") 

        let xDiag= diagA.every((x)=> x=== "X" || diagB.every((x)=>x==="X")
        let oDiag= diagA.every((x)=> x=== "O" || diagB.every((x)=>x==="O")
